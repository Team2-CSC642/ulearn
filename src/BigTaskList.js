        ...
        <Container className='overflow-auto'>
            {taskListData.map((task) => (
                <Row style={{ paddingBottom: '2%' }}>
                    <Col>
                        <Card key={task.name}>
                            <Card.Header onClick={() => setModal(true)} style={{ cursor: 'pointer', backgroundColor:task.color, color:'white'}}><h6>{task.class} {task.name} <PencilSquare style={{marginLeft:'.5%'}}/></h6></Card.Header>
